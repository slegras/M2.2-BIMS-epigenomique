...
3. Fill the form as such: 
  - reservation: 2530_epigenomics,
  - account: 2530_epigenomics, 
  - Partition: fast
  - CPUS: 2 
...
```{bash eval=FALSE}
cd /shared/projects/2530_epigenomics/
```
...
```{bash eval=FALSE}
cp -r /shared/projects/2530_epigenomics/training_data/data .
```

7. Your directory structure should be like this
 ```
/shared/projects/2530_epigenomics/<userid>
│
└───data
```
...
7. Go to the directory /shared/projects/2530_epigenomics/<userid>/1-QualityControl in the tree directory on the left of the jupyterhub window and double click on FNR_IP_ChIP-seq_Anaerobic_A_fastqc.html to visualize the file.
![alt text][fastqc]
...
sbatch -A 2530_epigenomics -p fast -o FNR_IP_ChIP-seq_Anaerobic_A.mapping.out --cpus-per-task 10 --wrap="bowtie2 -p 10 --mm -3 1 -x ../index/Escherichia_coli_K12 -U ../../data/FNR_IP_ChIP-seq_Anaerobic_A.fastq.gz -S FNR_IP_ChIP-seq_Anaerobic_A.sam"
```  
...
```{bash eval=FALSE}
cd /shared/projects/2530_epigenomics/<userid>/02-Mapping/bam
```
...
Go back to working home directory (i.e /shared/projects/2530_epigenomics/<userid>/)
```{bash eval=FALSE}
...
```{bash eval=FALSE}
cp /shared/projects/2530_epigenomics/training_data/03-ChIPQualityControls/fingerprint.png .
```
...
Go back to the working home directory (i.e /shared/projects/2530_epigenomics/\<login\>)
```{bash eval=FALSE}
## Unload deepTools
...
4. Remove IDR and MACS2 from your environment and go back to working home directory (i.e /shared/projects/2530_epigenomics/<userid>)
```{bash eval=FALSE}
...
setwd("/shared/projects/2530_epigenomics/<userid>/07-PeakAnnotation")
...
```{bash eval=FALSE}
cd /shared/projects/2530_epigenomics/<userid>
...
setwd("/shared/projects/2530_epigenomics/<userid>/07-PeakAnnotation-bonus")
