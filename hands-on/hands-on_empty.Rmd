...
$^1$ Plateau de Biologie Moléculaire et de Bioinformatique, INCI (Strasbourg)

$^2$ Équipe 1 – Modulation épigénétique des processus neurodégénératifs, LNCA (Strasbourg)
...
```{bash eval=FALSE}
cd /shared/projects/2635_master2_bims/
```
...
```{bash eval=FALSE}
cp -r /shared/projects/2635_master2_bims/training_data/data .
```

7. Your directory structure should be like this
 ```
/shared/projects/2635_master2_bims/${USER}
│
└───data
```
...
7. Go to the directory /shared/projects/2635_master2_bims/${USER}/1-QualityControl in the tree directory on the left of the jupyterhub window and double click on FNR_IP_ChIP-seq_Anaerobic_A_fastqc.html to visualize the file.
![alt text][fastqc]
...
There are multiple programs to perform the mapping step. For reads produced by an Illumina machine for ChIP-seq, the currently "standard" programs is Bowtie (versions 1 and 2)[@langmead_ultrafast_2009] [@langmead_fast_2012]. We will use **Bowtie version 2.5.5** for this exercise.

## - Bowtie
1. Load Bowtie
```{bash eval=FALSE}
module load bowtie2/2.5.5
```
2. Try bowtie
```{bash eval=FALSE}
bowtie2
...
```
6. Go back to the upper directory, i.e. 02-Mapping
```{bash eval=FALSE}
...
```{bash eval=FALSE}  
## Run alignment
## Tip: first type bowtie command line then add quotes around and prefix it with "sbatch -A 2635_master2_bims -p fast --cpus-per-task 10 --wrap="

sbatch -A 2635_master2_bims -p fast \
  -o FNR_IP_ChIP-seq_Anaerobic_A.mapping.out \
  --cpus-per-task 10 \
  --wrap="bowtie2 -p 10 --mm -3 1 -x ../index/Escherichia_coli_K12 -U ../../data/FNR_IP_ChIP-seq_Anaerobic_A.fastq.gz -S FNR_IP_ChIP-seq_Anaerobic_A.sam"
```  
...
```{bash eval=FALSE}
module unload samtools/1.23.1 bowtie2/2.5.5
```
...
```{bash eval=FALSE}
cd /shared/projects/2635_master2_bims/${USER}/02-Mapping/bam
```
...
**Run picard MarkDuplicates on the two other samples. How many duplicates are found in each sample?**

Go back to working home directory (i.e /shared/projects/2635_master2_bims/${USER}/)
```{bash eval=FALSE}
## Unload picard and samtools
module unload samtools/1.23.1 picard/2.23.5
...
```
4. If plotFingerprint takes too much time to run, take the file that has already been prepared for the training.
```{bash eval=FALSE}
cp /shared/projects/2635_master2_bims/training_data/03-ChIPQualityControls/fingerprint.png .
```
...
Go back to the working home directory (i.e /shared/projects/2635_master2_bims/${USER})
```{bash eval=FALSE}
## Unload deepTools
module unload deeptools/3.5.6
...
1. Download the following files from the server onto your computer
  ...
  * 02-Mapping/bam/Marked_FNR_IP_ChIP-seq_Anaerobic_A.bam
  * 02-Mapping/bam/Marked_FNR_IP_ChIP-seq_Anaerobic_A.bai
  * 02-Mapping/bam/Marked_FNR_IP_ChIP-seq_Anaerobic_B.bam
  * 02-Mapping/bam/Marked_FNR_IP_ChIP-seq_Anaerobic_B.bai  
  * 02-Mapping/bam/Marked_Anaerobic_INPUT_DNA.bam
  * 02-Mapping/bam/Marked_Anaerobic_INPUT_DNA.bai
...
5. Load the three bam files (Marked_FNR_IP_ChIP-seq_Anaerobic_A.bam, Marked_FNR_IP_ChIP-seq_Anaerobic_B.bam and Marked_Anaerobic_INPUT_DNA.bam) in IGV.
  * File / Load from File...
...
4. Remove IDR and MACS2 from your environment and go back to working home directory (i.e /shared/projects/2635_master2_bims/${USER})
```{bash eval=FALSE}
...
**Navigate through the genome to find peaks detected in the replicates (peak calling per replicate) and not found/kept with the IDR method** 
...
For the motif analysis, you first need to extract the sequences corresponding to the peaks. 
...
3. Create a bed file with the coordinates of the peak summits +/- 50 bp. 
To do this, first we use awk to generate the bed file with the coordinates of the peak summits. Then, we use bedtools slop to extend the peaks by 50 bp on each side. 
```{bash eval=FALSE}
## First Generate the bed file with the coordinates of the peak summits. 
awk

## Load samtools and create a file index of the genome fasta file. The file contains the chromosome sizes. This is needed for bedtools slop to work.
module load samtools/1.23.1
...
## Generate the bed file with the coordinates of the peak summits +/- 50 bp

```

4. Extract peak sequence in fasta format
```{bash eval=FALSE}
## First load bedtools
module load bedtools/2.31.1
...
MEME-ChIP is part of the MEME suite and combines several motif discovery and motif enrichment approaches. We will use it to identify sequence motifs enriched in the peak sequences and to compare them to known motifs.

1. Load MEME in your environment
```{bash eval=FALSE}
module load meme/5.5.7
```

2. Have a look at the help page of MEME-ChIP
```{bash eval=FALSE}
meme-chip -h
```

3. Launch MEME-ChIP on the peak sequences
  * `-oc`: output directory
  * `-db`: motif database used for motif comparison. Here we use the prokaryote motif database distributed with MEME.
  * `-meme-minsites` and `-meme-maxsites`: minimum and maximum number of sites to search. We restrict the search to motifs with 6 to 12 sites.
  * `-meme-nmotifs`: maximum number of motifs reported by MEME
  * the last argument is the input FASTA file containing the peak sequences
```{bash eval=FALSE}
meme-chip
```

4. Once the analysis is finished, list the result files
```{bash eval=FALSE}
ls FNR_anaerobic_memechip
```

5. Open the main HTML report named `meme-chip.html` from the directory `FNR_anaerobic_memechip` in the JupyterLab file browser, or download the result directory and inspect the report locally in a web browser.

**Is there anything interesting in MEME-ChIP results? If so, which motif is of interest and why (add screenshot of the results).**
...
1. Create a file suitable for annotatePeaks.pl. To run the tool needs a peak bed file composed of 6 fields (chr, start, end, name, score, strand). The 5 first columns of the file ../05-PeakCalling/idr/FNR_anaerobic_idr_peaks.bed are good but all other columns are of no use and the strand is missing. To generate a file with a correct format, we are using the tool cut to select fields 1 to 5 of the peak file and we add a "+" to every line using awk (*this is a code example that can do what we want, not the only solution to do so.*).
```{bash eval=FALSE}
cut \
  -f 1-5 \
  ../05-PeakCalling/idr/FNR_anaerobic_idr_peaks.bed | \
...
setwd("/shared/projects/2635_master2_bims/${USER}/07-PeakAnnotation")
...
colnames(d.annot)[2] <- "PeakID"  # name the 2nd column of the new file "PeakID"
...
**What are the possible gene features? (see in column Annotation - extract information like promoter-TSS, TSS, ...). Create a plot (pie chart, barplot...) showing the proportion of each of them (include both the plot and the code that created it in the report).**

6. Go back to working home directory (i.e /shared/projects/training/${USER}/M2.2-BIMS-epigenomique)
```{bash eval=FALSE}
## If you are in 07-PeakAnnotation

```

## - Functional enrichment analysis of the genes associated to the peaks
Use the Nearest.PromoterID column of the file FNR_anaerobic_idr_final_peaks_annotation.tsv to search for enriched functional annotations with the tool DAVID (Database for Annotation, Visualization and Integrated Discovery). Input the genes list on the DAVID website: https://davidbioinformatics.nih.gov/. 

Use *Escherichia coli str. K-12 substr. MG1655* as the background species. 
Use the tool *functional annotation*.

**Are there biological processes (GO) enriched in the list of genes associated to the peaks? Show the top results of the Functional Annotation Clustering.**
...
```{bash eval=FALSE}
cd /shared/projects/2635_master2_bims/${USER}
...
setwd("/shared/projects/2635_master2_bims/${USER}/07-PeakAnnotation-bonus")
```
