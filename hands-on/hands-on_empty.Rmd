...
## - Data availability
Data are publicly available as the GEO's experiment dataset GSE41187. Raw data were downloaded from [Sequence Read Archive (SRA)](http://www.ncbi.nlm.nih.gov/sra) and put on the server (see #faq). Samples of interest are FNR IP ChIP-seq Anaerobic A (GEO:GSM1010219 / SRA:SRR576933), FNR IP ChIP-seq Anaerobic B (GEO:GSM1010220 / SRA:SRR576934) and anaerobic INPUT DNA (GEO:GSM1010224 / SRA:SRR576938).
...
![alt text][jupebai]
3. Click on “Start”
4. In the launcher, click on "Terminal" in "Other" section. You are in your home directory by default. Check it:
```{bash eval=FALSE}
pwd
```
...
7. Your directory structure should be like this
 ```
/shared/projects/2321_m22_bims/<userid>
│
└───data
```
...
7. Go to the directory /shared/projects/2321_m22_bims/<userid>/1-QualityControl in the tree diretory on the left of the jupyterhub panel and double click on FNR_IP_ChIP-seq_Anaerobic_A_fastqc.html to visualize the file.
![alt text][fastqc]
...
## Use genome file: /shared/projects/2321_m22_bims/<userid>/data/Escherichia_coli_K12.fasta.gz
...
3. Let's see the parameters of bowtie before launching the mapping:
  ...
  * -q indicates the input file is in FASTQ format. FNR_IP_ChIP-seq_Anaerobic_A.fastq is the name of our FASTQ file.
  ...
  * Output file will be FNR_IP_ChIP-seq_Anaerobic_A.sam for the first replicate
  * USE 10 CPUS when running the mapping
```{bash eval=FALSE}  
## Run alignment
...
4. Sort the sam file and create a bam file using samtools [@li_sequence_2009]
  * -b: output BAM
```{bash eval=FALSE}
# First load samtools
module load samtools/1.15.1
# Then generate a bam file
## A bam file contains sorted data
## Use samtools sort and samtools view to generate the bam file
## Output file : FNR_IP_ChIP-seq_Anaerobic_A.bam for FNR_IP_ChIP-seq_Anaerobic_A

```

5. Create an index for the bam file with `samtools index`
```{bash eval=FALSE}

```

6. Compress the .sam file with gzip (you could also delete the file)
```{bash eval=FALSE}

```

**Analyze the result of the mapped reads:  
Open the file FNR_IP_ChIP-seq_Anaerobic_A.out (for example using the ` less ` command), which contains some statistics about the mapping. How many reads were mapped? How many multi-mapped reads were originally present in the sample? To quit less press 'q'**
...
**Analyze the result of the mapped reads:  
Open the file Anaerobic_INPUT_DNA.out. How many reads were mapped?**
...
```{bash eval=FALSE}
cd /shared/projects/2321_m22_bims/<userid>/02-Mapping/bam
```
2. Run Picard markDuplicates to mark duplicated reads (= reads mapping at the exact same location on the genome)
  ...
  * VALIDATION_STRINGENCY: Validation stringency for all SAM files read by picard. Use STRICT
```{bash eval=FALSE}
# Load picard
module load picard/2.23.5

# Run picard
## Output file: Marked_FNR_IP_ChIP-seq_Anaerobic_A.bam for FNR_IP_ChIP-seq_Anaerobic_A
...
module load samtools/1.15.1
## run samtools flagstat on Marked_FNR_IP_ChIP-seq_Anaerobic_A.bam

```

Go back to working home directory (i.e /shared/projects/2321_m22_bims/<userid>/)
```{bash eval=FALSE}
...
3. Run Deeptools [plotFingerprint](http://deeptools.readthedocs.io/en/latest/content/tools/plotFingerprint.html) [@ramirez_deeptools2:_2016] to draw the Lorenz curve
  ...
  * run it on a sample of 10000
```{bash eval=FALSE}
# Load deeptools in your environment
module load deeptools/3.5.0
# Run deeptools fingerprint: plotFingerprint
## output file: fingerprint_10000.png

```
4. If plotFingerprint takes ages to run. Take the file that has already been prepared for the training.
```{bash eval=FALSE}
cp /shared/home/slegras/2321_m22_bims/slegras/03-ChIPQualityControls/fingerprint.png .
```
...
4. Generate a scaled bigwig file on the IP with bamCoverage
  ...
  * --effectiveGenomeSize : size of the mappable genome. Use the size of full genome here: 4639675
  ...
  * output file name: SRRXXXXXX_nodup.bw (change SRRXXXXXX accordingly)
```{bash eval=FALSE}
...
There are multiple programs to perform the peak-calling step. Some are more directed towards histone marks (broad peaks) while others are specific to narrow peaks (transcription factors). Here we will use the callpeak function of MACS2 (version 2.2.7.1) because it's known to produce generally good results, and it is well-maintained by the developer.
...
```
5. Run macs2 for replicate A, then go to repB directory and run macs2 for replicate B by changing the treatment file (-t), the output file name (-n) and the log file (&>), this should take a few minutes each.

6. In a new directory called pool, run macs2 for the pooled replicates A and B by giving both bam files as input treatment files (-t). Use same parameters as for Macs2 on single replicate. Use FNR_Anaerobic_pool as output name.
```{bash eval=FALSE}
...
3. Run idr
```{bash eval=FALSE}
## output file name: FNR_anaerobic_idr_peaks.bed

```

4. Remove IDR and MACS2 from your environment and go back to working home directory (i.e /shared/projects/2321_m22_bims/<userid>)
```{bash eval=FALSE}
...
## First load samtools
module load samtools/1.15.1
## Create an index of the genome fasta file with samtools faidx


## First load bedtools
module load bedtools/2.30.0
## Extract fasta sequence from genomic coordinate of peaks with bedtools getfasta
## name output file: FNR_anaerobic_idr_peaks.fa
...
```{bash eval=FALSE}
## output file name: FNR_Anaerobic_A_summits+-100.bed

```
2. Extract the sequences for this BED file
```{bash eval=FALSE}
## Extract fasta sequence from genomic coordinate of peaks
## output file: FNR_Anaerobic_A_summits+-100.fa


## Compress the genome file with gzip as we won't need it anymore
...
```{bash eval=FALSE}
## Uncompress annotation file data/Escherichia_coli_K_12_MG1655.annotation.fixed.gtf.gz


## Uncompress genome file data/Escherichia_coli_K12.fasta.gz

```
2. Create a file suitable for annotatePeaks.pl. The file requires the strand information as a sixth column. As for chIP-seq peaks, strand information is meaningless, add "+" to every line. NOTE: fields are separated by TAB.
```{bash eval=FALSE}
...
annotatePeaks.pl peak/BEDfile genome > outputfile
	User defined annotation files (default is UCSC refGene annotation):
		...
		-gtf <gtf format file> (Use -gff and -gff3 if appropriate, but GTF is better). Use Escherichia_coli_K_12_MG1655.annotation.fixed.gtf.


4. Annotation peaks with nearby genes with Homer
```{bash eval=FALSE}
## Output file: FNR_anaerobic_idr_annotated_peaks.tsv
```

5. Open an Rstudio tab in Jupyterhub
![alt text][launchrstudio]

6. Add gene symbol annotation using R with Rstudio
```{R eval=FALSE, include=TRUE}

## set working directory
setwd("/shared/projects/2321_m22_bims/<userid>/07-PeakAnnotation")
## Or navigate using the "Files" tab and click on "More">"Set as Working Directory"

## read the file with peaks annotated with homer
## data are loaded into a data frame using read.table
## sep="\t": this is a tab separated file
## header: there is a line with headers (ie. column names)
d <-
...
## Open the file Escherichia_coli_K_12_MG1655.annotation.tsv.gz with read.table
...
## Keep the following columns 1,2,3,4,5,6,8,10,11
d.annot <-
...
```{bash eval=FALSE}
# sort | uniq -c to list and count occurrences of each item
...
Create a new folder 07-PeakAnnotation-bonus and go in it.
```{bash eval=FALSE}
cd /shared/projects/2321_m22_bims/<userid>
...
setwd("/shared/projects/2321_m22_bims/<userid>/07-PeakAnnotation-bonus")
```

```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
# Read the peak files with readPeakFile
peaks.forebrain <-
peaks.midbrain <-
peaks.limb <-
```
...
# display this as a barplot use colors defined in object col
...
# boxplot of the sizes use colors defined in object col
